...
class Hypothesis(NamedTuple):
    ...
    def asdict(self) -> dict:
        ...
        return self._replace(
            yseq=self.yseq.tolist(),  ## sequence of indices of the list of n_vocab characters
            score=float(self.score),  ## score of the sequence of characters
            scores={k: float(v) for k, v in self.scores.items()},  ## each type of score
        )._asdict()
...
class BeamSearch(torch.nn.Module):
    ...
    def score_full(
        ...
        scores = dict()
        states = dict()
        for k, d in self.full_scorers.items():  ## use each scorer independently
    ...
    def search(
        ...
        for hyp in running_hyps:
            ...
            scores, states = self.score_full(hyp, x)  ## score all posibilities for next character
            for k in self.full_scorers:
                weighted_scores += self.weights[k] * scores[k]  ## use all scorers with respective weights
            # partial scoring
            if self.do_pre_beam:
                pre_beam_scores = (
                    weighted_scores
            ...
            # update hyps
            for j, part_j in zip(*self.beam(weighted_scores, part_ids)):  ## which character to add? choose only best k
                # will be (2 x beam at most)
                best_hyps.append(
                    Hypothesis(
                        score=weighted_scores[j],  ## score of sentence with character j added
