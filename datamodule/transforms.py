...
## this is only so that windows doesn't complain. shouldn't be needed in the cluster
def divide(x):
    return x / 255.0
...
class TextTransform:
    ...
    def __init__(
        ...
        hashmap_keys = [unit.split()[0] for unit in units]  ## deterministic order
        self.token_list = ["<blank>"] + hashmap_keys + ["<eos>"]
        ### CHANGE
        self.ignore_id = -1
    ...
    def post_process(self, token_ids):
        ...
        text = text.replace("\u2581", " ").strip()  ## many tokens are like "▁hello"
        return text

    def _ids_to_str(self, token_ids, char_list):
        token_as_list = [char_list[idx] for idx in token_ids]  ## <blank> is never removed??
        return "".join(token_as_list).replace("<space>", " ")  ## <space> is not in list??
